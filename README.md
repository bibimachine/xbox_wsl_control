# xbox_joy — 雷神 G30S 手柄 ROS 2 包

在 WSL2 中使用雷神 G30S 手柄（有线模式），将手柄输入发布为 `/joy` 话题，并打印可读日志。
...
# 编译并启动（首次或改动后）
cd xbox_control
colcon build --packages-select xbox_joy --symlink-install
source install/setup.bash
ros2 launch xbox_joy xbox_joy.launch.py
